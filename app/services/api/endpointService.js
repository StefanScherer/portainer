angular.module('portainer.services').factory('EndpointService', [
  '$q',
  'Endpoints',
  'FileUploadService',
  function EndpointServiceFactory($q, Endpoints, FileUploadService) {
    ...
    service.updateAccess = function(id, authorizedUserIDs, authorizedTeamIDs) {
      return Endpoints.updateAccess(
        { id: id },
        {
          ...
          authorizedTeams: authorizedTeamIDs
        }
      ).$promise;
    };
    ...
    service.updateEndpoint = function(id, endpointParams) {
      ...
      if (endpointParams.type && endpointParams.URL) {
        query.URL = endpointParams.type === 'local'
          ? 'npipe://' + endpointParams.URL
          : 'tcp://' + endpointParams.URL;
      }

      var deferred = $q.defer();
      FileUploadService.uploadTLSFilesForEndpoint(
        id,
        endpointParams.TLSCACert,
        endpointParams.TLSCert,
        endpointParams.TLSKey
      )
